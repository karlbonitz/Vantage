...
local byName = {
    -- heal
    ["dark mending"] = { castTime = 2, interruptible = true, priority = 5, category = "heal", note = "Channeler self/cross-heal (~69-80k), 2s cast - must be kicked every cast or channelers never die. Assign 2 interrupters per channeler (Dark Mending + Shadow Bolt Volley come back-to-back). spellID 30528 verified." },
    ["greater heal"] = { castTime = 3, interruptible = true, priority = 5, category = "heal", note = "Large single-target heal - TOP kick target everywhere it appears (2-3.5s cast, interruptible). SSC/TK casters: Coilfang Priestess (~40-50k heal), Bloodwarder Mender, Sister of Pleasure, Solarium Priest (heals Solarian on spawn - kick+stun or she resets). EXCEPTIONS: Coilfang Priestess keeps healing ~15s in Spirit of Redemption on death; Sister of Pleasure is kick-immune under her Shell of Life (kill/purge first); instant shields/Holy Nova are not cast-interruptible." },
    ["heal"] = { castTime = 2.5, interruptible = true, priority = 5, category = "heal", note = "Merged generic single-target Heal across many healer mobs/bosses; all verified interruptible. Cast time varies 1.6-3.5s by mob. CAVEAT: Blindeye becomes interrupt-IMMUNE while his Greater PW:Shield is up (silence-type interrupts still land). Top kick target wherever it appears." },
    ["healing wave"] = { castTime = 1.5, interruptible = true, priority = 5, category = "heal", note = "The single most important kick in SSC. ~Every 15s, heals >100k with NO LoS/range limit (heals all 4 bosses anywhere). Dedicated interrupt rotation required; Curse of Tongues / Mind-numbing Poison helps." },
    ["holy light"] = { castTime = 2.5, interruptible = true, priority = 5, category = "heal", note = "Big paladin/healer heal, verified interruptible by kick/Counterspell/Pummel. Note Skarloc cannot be Silenced (use a hard kick)." },
    ...
    ["repair"] = { castTime = 0, interruptible = true, priority = 5, category = "heal", note = "CHANNELED repair (~1050/2s on the boss). Breaks on ANY damage to the mechanic (kick, stun, or just hitting it). Adds spawn at 75/50/25%." },
    ["bandage"] = { castTime = 1, interruptible = true, priority = 4, category = "heal", note = "1s CAST heal-over-time (~1200/8s), NOT a channel - a kick stops the cast. Physician also has Anesthetic (sleep) and Holy Shock (heroic). spellID 38919 verified." },
    ["eternal affection"] = { castTime = 2, interruptible = true, priority = 4, category = "heal", note = "Julianne's ~2s Holy heal (46k-54k) - top interrupt priority. She is immune to Silence but NOT to Kick/Shield Bash/Wind Shear." },
    ["tranquility"] = { castTime = 0, interruptible = false, priority = 4, category = "heal", note = "DO NOT KICK - channeled AoE heal, uninterruptible on both known casters: Freywinn (Botanica, Tree Form / damage-immune, stopped only by killing the 3 Frayer Protectors) and Tidewalker Depth-Seer (SSC murloc, chaincasts at low HP - DPS through it). Marker so the addon won't suggest a wasted kick." },
    ["flash of light"] = { castTime = 1.5, interruptible = false, priority = 2, category = "heal", note = "DO NOT KICK - verified un-interruptible on every known caster; DPS them down. TK: Bloodwarder Vindicator/Squire & Crimson Hand Blood Knights cast a kick-IMMUNE Flash of Light that heals only ~3% HP (their Cleanse / Hammer of Justice are separate instant casts). Marker so the addon won't recommend a wasted kick." },
    ["healing touch"] = { castTime = 3.5, interruptible = true, priority = 4, category = "heal", note = "Big single-target heal, interruptible. Casters: Tidewalker Depth-Seer (SSC murloc, ~12k heal - KICK this; only its Tranquility resists) and Spectral Chargers/Stallions (Karazhan). Kick on sight." },
    ...
    ["domination"] = { castTime = 3, interruptible = true, priority = 5, category = "cc", note = "Mind-control on a party member - a top-tier stop (MC'd player on the healer can wipe). The Maker's is verified interruptible/#1 priority. CAVEATS: Soothsayer cast-bar behavior in 2.5.x unverified; Pathaleon spell 35280, but a single ID can't cover all four mobs so spellID omitted. PvP trinket also breaks the MC." },
    ["fear"] = { castTime = 1.5, interruptible = true, priority = 4, category = "cc", note = "Warlock-style trash Fears (Darkcaster, Sethekk Prophet) are verified interruptible 1.5s casts - dangerous because feared players pull extra packs. CAVEAT: boss/AoE-fear variants (O'mrogg, Siren, channeler) are NOT firmly confirmed interruptible and some are treated as instant - verify in-game; keep Fear Ward/Tremor as backup." },
    ...
    -- summon
    ["summon cabal deathsworn"] = { castTime = 2, interruptible = true, priority = 5, category = "summon", note = "Top interrupt priority on the pull to avoid being overrun by adds. spellID 33506 verified (single source)." },
    ["summon arcane golem"] = { castTime = 2, interruptible = true, priority = 4, category = "summon", note = "Summons add(s); interrupt to prevent the extra mob. Netherbinder also has Arcane Nova/Starfire/Dispel Magic. spellID 35251 verified (single source)." },
    ...
    ["summon ethereal wraith"] = { castTime = 2, interruptible = true, priority = 3, category = "summon", note = "Summon add; interrupt to avoid being overrun. Spellbinder also has Counterspell and Immolate." },
    ...
    ["arcane missiles"] = { castTime = 5, interruptible = true, priority = 5, category = "nuke", note = "THE kick target on Aran - 5s channel, ~6300-7700 arcane. Interrupt until he runs dry. WARNING: his raid-wide Counterspell (10s lockout, all within 10yd) punishes melee positioning." },
    ["blast nova"] = { castTime = 0, interruptible = false, priority = 5, category = "nuke", note = "DO NOT KICK - raid-wide channel (~10s) every ~50-60s, stopped ONLY by 5 players channeling the Manticron Cubes. Correctly flagged uninterruptible so the addon never prompts a kick. spellID 30616 verified." },
    ["pyroblast"] = { castTime = 4, interruptible = true, priority = 5, category = "nuke", note = "Massive fire nuke (~50k on Kael, near one-shot). Interruptible EXCEPT while Kael's Shock Barrier is up (burn the barrier, then kick the Pyroblasts). Rift Keeper's version is a slow ~6s cast - easy kick." },
    ["shadow bolt volley"] = { castTime = 2.5, interruptible = true, priority = 5, category = "nuke", note = "Party/raid-wide shadow AoE. Generally interruptible. CAVEAT: Keli'dan's version is poorly documented and some report resists (verify). Conflicting NPC spellIDs exist (channeler 30510 vs others 39175) so spellID omitted on this merge." },
    ...
    ["chain lightning"] = { castTime = 2.5, interruptible = true, priority = 3, category = "nuke", note = "Multi-target nature nuke, interruptible on Syth and The Crone. CAVEAT: The Black Stalker's (boss, p4) interruptibility is NOT confirmed by any source - do not rely on a kick landing there." },
    ...
    ["solarburn"] = { castTime = 2, interruptible = true, priority = 3, category = "nuke", note = "Fire damage + DoT, interruptible. Astromage also has Scorch and Fire Shield." },
    ["arcane bolt"] = { castTime = 2.5, interruptible = true, priority = 2, category = "nuke", note = "Interruptible nuke; reduces incoming damage during portal defense. Also Arcane Explosion in melee." },
    ...
    ["greater fireball"] = { castTime = 3, interruptible = true, priority = 2, category = "nuke", note = "~9k fire nuke, 3s cast, technically interruptible. In practice Krosh is tanked by a Spellsteal mage who deliberately does NOT kick it - keep priority low. His Blast Wave is instant, not a kick target." },
    ["holy fire"] = { castTime = 2, interruptible = true, priority = 2, category = "nuke", note = "Direct fire damage + DoT, interruptible. On Catriona her Greater Heal is the real priority (kick that first). CAVEAT: Maiden's 1s-cast version is hard to kick and her DoT is usually dispelled rather than interrupted - low value there." },
    ["sonic boom"] = { castTime = 1.5, interruptible = false, priority = 2, category = "nuke", note = "DO NOT KICK - telegraphed run-out/LoS mechanic, treated as uninterruptible in practice. Marker so players don't burn a kick." },
    ...
    ["blade dance"] = { castTime = 0, interruptible = false, priority = 1, category = "other", note = "DO NOT KICK - channeled whirlwind/charge AoE, not interruptible. Use defensives/spread. Marker so the addon stays silent." },
    ["evocation"] = { castTime = 20, interruptible = false, priority = 1, category = "other", note = "DO NOT KICK - 20s channel after the 10th Astral Flare; Curator takes triple damage (burn window). Marker so the addon stays silent." },
    ...
    ["holy smite"]       = { castTime = 2.5, interruptible = true,  priority = 2, category = "nuke", note = "Solarium Priest (Solarian adds, TK The Eye) filler nuke ~675-1025 Holy, ~2.5s. Interruptible but LOW value vs their Greater Heal - only kick if no heal is up. Name-keyed; other priest casters reuse it." },
    ["scorch"]           = { castTime = 1.5, interruptible = true,  priority = 2, category = "nuke", note = "Sunseeker Astromage (Mechanar) fire nuke; kick to lock the Fire school and deny the follow-up Solarburn. The mob's Fire Shield is an instant reflect buff on an ally - PURGE it, not a kick target." },
    ...
    ["recharge"]         = { castTime = 0,   interruptible = false, priority = 1, category = "heal", note = "DO NOT KICK - Crystalcore Mechanic (TK, Void Reaver's room) channeled heal-bot ~10k HPS/10s, ignores LoS, UNINTERRUPTIBLE. Mechanics are Demons -> BANISH/CC + kill. Marker so players don't waste a kick." },
    ["chaos blast"]      = { castTime = 0,   interruptible = false, priority = 3, category = "nuke", note = "DO NOT KICK - Leotheras the Blind, Demon Form (SSC). Binary Fire AoE + stacking fire-taken debuff; beaten with Fire Resistance + a geared demon-form tank, NEVER by kicking. castTime 0 = no kickable bar, not a channel." },
    ["cataclysmic bolt"] = { castTime = 0,   interruptible = false, priority = 2, category = "nuke", note = "DO NOT KICK - Fathom-Lord Karathress (SSC). Instant ~50% max-HP shadow hit + brief knockdown on a random mana-user; no cast bar to interrupt; healers spot-heal the victim. castTime 0 = instant. spellID 38441 verified (Karathress-only)." },
    ["tidal wave"]       = { castTime = 2,   interruptible = false, priority = 2, category = "nuke", note = "DO NOT KICK - Morogrim Tidewalker (SSC). 2s-cast frontal Frost cone + attack-speed slow; HAS a cast bar so it looks kickable, but it is a positional cleave (face Morogrim into a wall), not an interrupt. No source confirms a kick works." },
    ...
    ["flame strike"]     = { castTime = 3,   interruptible = false, priority = 1, category = "nuke", note = "DO NOT KICK - Kael'thas (p4, TK) ground-targeted fire patch: move out, don't kick. Save interrupts for his Fireball/Pyroblast (kickable except under Shock Barrier). Distinct key from the kickable mage-trash 'flamestrike' (one word)." },
